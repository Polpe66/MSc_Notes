...
L'analisi di una specifica istanza di training, definita dalla query e dai rispettivi documenti ($d_2, d_3, \dots, d_{|q|}$), rivela un comportamento elegante. Se i due documenti sono già ordinati correttamente in partenza (condizione in cui $o_{ij} \to +\infty$), il gradiente sfuma fisiologicamente verso lo zero ($\lambda_{ij} \to 0$). 
